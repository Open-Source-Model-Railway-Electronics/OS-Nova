...
V implement loconet SLOT manager for throttles
...
V TEST loconet to set accessories
...
enum trainCommands
{
    _128_STEPS          = 0b00111111 ,
    FORWARD             = 0b011  << 5, // not in use
    DRIVE               = 0b010  << 5, // not in use
    ...
    POM_WRITE           = 0b11101100 ,
    CV_WRITE            = 0b01111100 ,
    // F37_F44    = 0b11011001 , we do not believe in this BS who needs more than F36 functions
    // F45_F52    = 0b11011010 , 
    // F53_F60    = 0b11011011 , 
    // F61_F68    = 0b11011100 , 
} ;   
...
uint8   repeatCounter ;
uint8   repeatCount ;
uint8   resetPacketNeeded ;
uint8   CV_packetNeeded ;
uint8   pomPacketNeeded ;
uint16  CV ;
uint8   val ;
...
// LOCOMOTIVE COMMAND FUNCTIONS
void setSpeedDir(  uint8_t currentSlot, uint8 speed, uint8 dir )
{
    //uint8 currentSlot = getSlot( address );

    if( speed >= 1 ) speed ++ ;

    slot[currentSlot].dir   =   dir ;
    slot[currentSlot].speed = speed ;
    slot[currentSlot].newInstruction |= newSpeed ;
    slot[currentSlot].set = 1 ;

    setRepeatAmount() ;
}

void setFunction( uint8_t currentSlot, uint8 Fx, uint8 state ) // note may directly insert the data from loconet.
{
    //uint8 currentSlot = getSlot( address ) ;
    
    uint8 *bank ; // ALL SEEMS TO WORK HERE!!
         if( Fx >=  0 && Fx <=  4 ) { if( -- Fx == 255 )  Fx = 4 ;  // F0 gets put on the 4th spot, while F1-F4 are set at spots 0-3 
...
void writeCv( uint16_t _CV, uint8_t _val )
{
    CV                  =  _CV - 1 ;
    val                 =     _val ;
    resetPacketNeeded   =        1 ;
...
void writePom( uint8_t slot, uint16_t _CV, uint8_t _val )
{
    currentSlot         =     slot ;
    CV                  =  _CV - 1 ;
    val                 =     _val ;
...
/******** STATE FUNCTIONS ********/
// in order of priority we check for:
// if an instructions is to be/being repeated
// if a reset packet is due
// if a service packet is due
// if an accessory is set
// if a loco has new instructions
...
StateFunction( selectNewPacketType )
{
    ...
    if( resetPacketNeeded ) 
    {   resetPacketNeeded = 0 ;

        packetType = resetPacket ;
        return 1 ;
    }

    if( CV_packetNeeded ) // after transmission, CV will be cleared to 0
    {   CV_packetNeeded = 0 ;

        packetType    = servicePacket ;
        repeatCounter = nServiceRepeats ;
        return 1 ;
    }
...
/******   Service Packet   *******/  
    if( packetType == resetPacket ) 
    {
        buffer[toFill].data[index++] = 0x00 ;   // address byte
        buffer[toFill].data[index++] = 0x00 ;   // instruction = reset
    }

    else if( packetType == servicePacket )
    {
        buffer[toFill].data[index++] = CV_WRITE | ((CV >> 8) & 0x03) ;
        buffer[toFill].data[index++] = (CV & 0xFF) ;
        buffer[toFill].data[index++] = val ;
    }
    
/******   LOCO ADDRESS    *******/
    else if( packetType < accessoryPacket )       // no accessory is due, go do locomotive
    {
    ...
    case pomPacket:
    {
        buffer[toFill].data[index++] = POM_WRITE | ((CV >> 8) & 0x03); // 1110 11 = POM write byte
        buffer[toFill].data[index++] = CV & 0xFF;
        buffer[toFill].data[index++] = val;
        break;
    }

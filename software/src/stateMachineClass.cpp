...
void StateMachine::setTimeout_us( uint32_t time2run )
{
    prevTime = micros() ;
    interval = time2run ;
    timeOutSet = false ;
...
uint8 StateMachine::timeout_us()
{
    if( (micros() - prevTime) >= interval && timeOutSet == false )
    {
        timeOutSet = true ; 
        return 1 ;
    }
...
uint8 StateMachine::repeat_us( uint32 _interval )
{
    if( micros() - prevTime >= _interval )
    {
        prevTime = micros() ;
        return true ;
    }
